...
let n = Phaser.Math.Between(1, 300);
let choices = [];
let question ='';
let answer='';
let questionId=-1;
...
export default class Question extends Phaser.Scene
{
    ...
    create ()
    {
        ...
        setTimeout(() => {
            ...
            const question_test=this.add.text(this.bg.x-this.bg.width/8, this.bg.y-this.bg.height/4-20, question, { fontFamily: 'Arial',fontSize: '24px', fill: '#ffffff',wordWrap: { width: 400, useAdvancedWrap: true }});
            ...
            for (let i = 0; i < num_choice; i++) {
                this.add.image(this.bg.x-this.bg.width/8-20, this.bg.y-this.bg.height/4+(i+2)*55+20,q_type+"_yes").setScale(0.2);
                select_box[i]=this.add.image(this.bg.x-this.bg.width/8-20, this.bg.y-this.bg.height/4+(i+2)*55+20,q_type+"_no").setScale(0.2).setAlpha(1).setInteractive();
                ...
                this.add.text(this.bg.x-this.bg.width/8, this.bg.y-this.bg.height/4+(i+2)*55+5, choices[i], { fontFamily: 'Arial', fontSize: '24px', fill: '#ffffff',wordWrap: { width: 500, useAdvancedWrap: true } });
            }

            var report_button = this.add.text(
                this.bg.x-this.bg.width/8-130,
                this.bg.y-this.bg.height/4+270,
                'Report\nQuestion',
                {fontFamily: 'Arial', fontSize: '20px', fill: 'red' }
                );
            report_button.setInteractive();
                    
            report_button.once('pointerdown', function() {
                report_button.setText('Reported');
                fetch('http://3.96.64.25:9090/fisherman/GPT/submitReport', {
            ...
            confirm_button.once('pointerdown', () => {
                ...
                if (JSON.stringify(my_idx)!==JSON.stringify(answer_idx)){
                    ...
                    confirm_button.setAlpha(0);
                    answer_text.setColor('red').setAlpha(1);
                    back_button.setColor('blue')
                }else{
